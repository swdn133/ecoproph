...
def build_prophet_dataframe_sum(input_data, value_columns: list):
    ...
    prophet_df['sun'] = input_data['SD_SO']
...
def get_sunshine(x, dwd_df):
    timestring = x.strftime("%Y-%m-%d %H:%M:%S")
    return dwd_df.loc[timestring]['SD_SO']
...
def get_dwd_df():
    sun_df = dwd_utils.load_dwd_dataframe('C:/workspace/produkt_sd_stunde_19920517_20181231_01262.txt')
    sun_df = dwd_utils.prepare_sd_so_dataframe(sun_df, '20170101')
    sun_future_df = dwd_utils.load_dwd_dataframe('C:/workspace/produkt_sd_stunde_20180602_20191203_01262.txt')
    sun_future_df = dwd_utils.prepare_sd_so_dataframe(sun_future_df, '20190101')
    sun_df = sun_df.append(sun_future_df)

    temp_df = dwd_utils.load_dwd_dataframe('C:/workspace/produkt_tu_stunde_19920517_20181231_01262.txt')
    temp_df = dwd_utils.prepare_tt_tu_dataframe(temp_df, '20170101')
    temp_future_df = dwd_utils.load_dwd_dataframe('C:/workspace/produkt_tu_stunde_20180529_20191129_01262.txt')
    temp_future_df = dwd_utils.prepare_tt_tu_dataframe(temp_future_df, '20190101')
    temp_df = temp_df.append(temp_future_df)

    dwd_df = pd.merge(temp_df, sun_df, how='left', on=['YYYYMMDD', 'Hour', 'timestring'])

    dwd_df['SD_SO'] = dwd_df['SD_SO'].apply(lambda x: replace_nan(x, 0.0))
...
def main():
    ...
    try:
        ...
        with open('C:\\workspace\\ecoproph\\ecoproph_experimentalPvModel3BothA100.pckl', 'rb') as fin:
        ...
        model = Prophet(yearly_seasonality=6, weekly_seasonality=False, daily_seasonality=5, 
        ...
        model.add_regressor('sun', mode="additive", standardize=False, prior_scale=100)
        print("Model fitting...")
        start = datetime.now()
        model.fit(prophet_df)
        print("Time elapsed [seconds]: ", (datetime.now() - start).total_seconds())
        with open('C:\\workspace\\ecoproph\\ecoproph_experimentalPvModel3BothA100.pckl', 'wb') as fout:
    ...
    future['sun'] = future['ds'].apply(lambda x: get_sunshine(x, dwd_df))
    dwd_df = dwd_df.reset_index()
    print("Predict future for 365 days...")
    start = datetime.now()
    forecast = model.predict(future)

    with open('C:\\workspace\\ecoproph\\forecast_experimentalPvModel3BothA100.pckl', 'wb') as fout:
